...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                #checks if ant is a container and if so, no ants are inside and insect is not a container
                if self.ant.can_contain(insect):
                    self.ant.contain_ant(insect)
                #checks if insect is a container and there are no ants and the ant itself isn't a container
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            #removes self.ant
                if hasattr(self.ant, 'impostor') and not self.ant.impostor:
                    return
                elif hasattr(self.ant, 'container') and self.ant.container:
                    self.ant = self.ant.ant
                else:
                    self.ant = None
            else:
            #removes the inner ant
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if hasattr(insect, 'impostor') and not insect.impostor:
                        return
                    else:
                        self.ant.ant = None
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        distance_thrower = 0
        while (place != hive) and (not self.place.bees):
            if place.bees and distance_thrower >=self.min_range and distance_thrower<=self.max_range:
                self.place.bees = place.bees
...
class LongThrower(ThrowerAnt):
    ...
    max_range = 10
    # BEGIN Problem 4B
    return super().nearest_bee(hive, self.min_range, self.max_range)
    implemented = True   # Change to True to view in the GUI
    # END Problem 4B
...
class ShortThrower(ThrowerAnt):
    ...
    max_range = 3
    # BEGIN Problem 4B
    return super().nearest_bee(hive, self.min_range, self.max_rang
    implemented = True   # Change to True to view in the GUI
    # END Problem 4B
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 8
        for index in range(len(self.place.bees)-1, -1, -1):   #decreases index each time; goes backwards in the list to avoid the removal problem
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting:
            self.digesting-=1
        elif random_or_none(self.place.bees):
...
class BodyguardAnt(Ant):
    ...
    implemented = True
    container = True
    ant = None   # Change to True to view in the GUI
    # END Problem 7
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        for index in range(len(self.place.bees)-1,-1,-1):
            self.place.bees[index].reduce_armor(self.damage)
        super().action(colony)
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            while place is not None:
                if place.ant:
                    if not place.ant.double_ants:
                        place.ant.damage *= 2
                        place.ant.double_ants = True
                    if isinstance(place.ant, BodyguardAnt) and place.ant.ant and not place.ant.ant.double_ants:
                        place.ant.ant.damage *= 2
                        place.ant.ant.double_ants = True
                place = place.exit
            ThrowerAnt.throw_at(self, ThrowerAnt.nearest_bee(self, colony.hive))
        #
    def reduce_armor(self, amount):
...
def apply_effect(effect, bee, duration):
    ...
    pre_act_effect = effect(prev_action)
    def appied(colony):
        nonlocal duration
        if duration>0:
            pre_act_effect(colony)
        else:
            prev_action(colony)
        duration -=1
    bee.action = appied
